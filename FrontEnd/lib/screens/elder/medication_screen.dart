...
class _MedicationScreenState extends State<MedicationScreen> {
  ...
  Future<void> _editMed(DocumentSnapshot<Map<String, dynamic>> doc) async {
    ...
    // 기존 문서값 반영
    final List<String> initialTimes = (data['times'] as List?)?.cast<String>() ?? [];
    final Set<int> initialDows = ((data['daysOfWeek'] as List?)?.cast<int>() ?? []).toSet();
    DateTime? startAt = (data['startAt'] as Timestamp?)?.toDate();
    DateTime? endAt = (data['endAt'] as Timestamp?)?.toDate();

    // 편집용 가변 상태
    final List<String> times = [...initialTimes]..sort();
    final Set<int> selectedDows = {...initialDows};

    String _fmtDate(DateTime? d) =>
        d == null ? '선택 안됨' : DateFormat('yyyy-MM-dd').format(d);

    Future<void> _addTime(BuildContext ctx, StateSetter setSheetState) async {
    ...
    void _removeTime(String t, StateSetter setSheetState) {
      times.remove(t);
      setSheetState(() {});
    }

    Future<void> _pickStart(BuildContext ctx, StateSetter setSheetState) async {
    ...
    Future<void> _pickEnd(BuildContext ctx, StateSetter setSheetState) async {
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        const Text('복용 시간', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
                        TextButton.icon(
                          onPressed: () => _addTime(ctx, setSheetState),
                    ...
                    // 요일 선택
                    const Text('요일 선택', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
                    Wrap(
                      spacing: 8,
                      children: List.generate(7, (i) {
                        final selected = selectedDows.contains(i);
                        return FilterChip(
                    ...
                    Container(
                      ...
                      child: Column(
                        children: [
                          ListTile(
                            title: Text('복용 시작일: ${_fmtDate(startAt)}'),
                            trailing: const Icon(Icons.calendar_today),
                            onTap: () => _pickStart(ctx, setSheetState),
                          ),
                          const Divider(height: 1),
                          ListTile(
                            title: Text('복용 종료일: ${_fmtDate(endAt)}'),
                            trailing: const Icon(Icons.calendar_today),
                            onTap: () => _pickEnd(ctx, setSheetState),
                          ),
                        ],
                      ),
                    ),
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        ...
                        onPressed: () async {
                          ...
                          if (times.isEmpty) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(content: Text('복용 시간을 하나 이상 추가해 주세요.')),
                            );
                            return;
                          }
                          ...
                          await doc.reference.update({
                            'name': name,
                            'times': times, // 여러 시간 저장
                            'daysOfWeek': selectedDows.toList()..sort(), // 0=일..6=토 (비면 매일 처리)
